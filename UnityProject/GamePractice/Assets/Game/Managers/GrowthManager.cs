...
namespace Sayne
{
    ...
    public class GrowthManager : ManagerBase
    {
        ...
        private static readonly Dictionary<string, int> ExpPerKill = new Dictionary<string, int>
        {
            [EnemyID.Goblin] = 8,
            [EnemyID.Ogre] = 30,
            [EnemyID.ArmedOgre] = 38,
            [EnemyID.OgreBoss] = 200,
        };
        ...
        private readonly Dictionary<GrowthStat, ReactiveProperty<int>> _statLevels =
            new Dictionary<GrowthStat, ReactiveProperty<int>>();
        ...
        public ReadOnlyReactiveProperty<int> StatLevel(GrowthStat stat) => _statLevels[stat];

        /// <summary>지금 이 항목을 한 칸 올리는 값. 레벨이 오를수록 비싸진다 — 수식은 GrowthPlan 이 가진다.</summary>
        public long CostToUpgrade(GrowthStat stat) => GrowthPlan.CostToUpgrade(stat, _statLevels[stat].Value);

        public bool IsMaxLevel(GrowthStat stat) => _statLevels[stat].Value >= GrowthPlan.MaxLevel(stat);
