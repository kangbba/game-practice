using R3;
using UnityEngine;

namespace Sayne
{
    /// <summary>
    /// 무엇을 떨어뜨리고 주우면 무슨 일이 나는지를 정하는 쪽. 적이 죽으면 그 적의 설계값을 굴려,
    /// 종류에 맞는 그림과 효과를 얹어 DropManager 에 스폰을 부탁한다.
    /// 재화·가방·장비 아이콘을 아는 건 여기까지고, 구슬을 뿌리는 쪽은 그걸 모른다.
    /// </summary>
    public class DropDirector : ManagerBase
    {
        private readonly EnemyManager _enemyManager;
        private readonly DropManager _dropManager;
        private readonly CurrencyManager _currencyManager;
        private readonly HeroManager _heroManager;
        private readonly EquipmentManager _equipmentManager;
        private readonly ItemAssetManager _itemAssets;

        public DropDirector(EnemyManager enemyManager, DropManager dropManager, CurrencyManager currencyManager,
            HeroManager heroManager, EquipmentManager equipmentManager, ItemAssetManager itemAssets)
        {
            _enemyManager = enemyManager;
            _dropManager = dropManager;
            _currencyManager = currencyManager;
            _heroManager = heroManager;
            _equipmentManager = equipmentManager;
            _itemAssets = itemAssets;
        }

        protected override void OnInit()
        {
            _enemyManager.Died
                .Subscribe(this, (enemy, self) => self.DropFrom(enemy))
                .RegisterTo(LifeToken);
        }

        protected override void OnRelease()
        {
        }

        private void DropFrom(Enemy enemy)
        {
            foreach (var entry in enemy.Plan.Roll())
            {
                Request(entry, enemy.transform.position);
            }
        }

        /// <summary>선언 한 줄을 그림과 효과로 옮긴다. 설계값의 모양이 밖으로 새는 건 이 함수까지다.</summary>
        private void Request(DropItemData entry, Vector3 position)
        {
            if (entry.Type == DropType.Gold)
            {
                // 지역 변수로 받아 둔다 — 나중에 실행될 때 설계값을 다시 들여다보지 않게.
                var gold = entry.GoldAmount;
                _dropManager.Spawn(position, _itemAssets.CoinIcon, $"Gold {gold}",
                    _ => _currencyManager.AddGold(gold));
                return;
            }

            // 장비 아이콘은 장비 자기 것이다. 가방은 영웅들이 함께 쓰는 하나라 누가 주웠는지는 안 본다.
            var equipmentID = entry.EquipmentID;
            _dropManager.Spawn(position, _equipmentManager.GetIcon(equipmentID), equipmentID,
                _ => _heroManager.Inventory.Add(equipmentID));
        }
    }
}
