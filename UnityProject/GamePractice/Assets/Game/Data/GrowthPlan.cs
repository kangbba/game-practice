...
namespace Sayne
{
    ...
    public static class GrowthPlan
    {
        ...
        private static readonly Dictionary<GrowthStat, Entry> Entries = new Dictionary<GrowthStat, Entry>
        {
            [GrowthStat.AttackPower] = new Entry("공격력", gainPerLevel: 2, baseCost: 50, costGrowth: 1.18f, maxLevel: 99),
            [GrowthStat.MaxHP] = new Entry("체력", gainPerLevel: 10, baseCost: 40, costGrowth: 1.15f, maxLevel: 99),
        };

        /// <summary>화면에 그리는 순서이자 저장 순서. 항목이 늘면 여기와 Entries 둘 다에 적는다.</summary>
        public static readonly GrowthStat[] All = { GrowthStat.AttackPower, GrowthStat.MaxHP };

        public static string DisplayName(GrowthStat stat) => Entries[stat].DisplayName;

        public static int MaxLevel(GrowthStat stat) => Entries[stat].MaxLevel;

        /// <summary>지금 레벨에서 다음 레벨로 올리는 값. 레벨이 오를수록 CostGrowth 배씩 비싸진다.</summary>
        public static long CostToUpgrade(GrowthStat stat, int currentLevel)
        {
        ...
        public static CharacterStats BonusFor(GrowthStat stat, int level)
        {
            var gain = Entries[stat].GainPerLevel * (level - 1);

            return stat switch
            {
                GrowthStat.AttackPower => new CharacterStats(maxHP: 0, moveSpeed: 0f, attackPower: gain),
                GrowthStat.MaxHP => new CharacterStats(maxHP: gain, moveSpeed: 0f, attackPower: 0),
                _ => default,
            };
        }

        /// <summary>스탯 묶음에서 이 항목에 해당하는 수치만 꺼낸다. 창이 "기본 n (+ 성장 m)" 을 그릴 때 쓴다.</summary>
        public static int ValueOf(GrowthStat stat, CharacterStats stats)
        {
            return stat switch
            {
                GrowthStat.AttackPower => stats.AttackPower,
                GrowthStat.MaxHP => stats.MaxHP,
                _ => 0,
            };
        }
    }
