...
{
    /// <summary>
    /// 튜토리얼 대사 프리팹 두 개를 만든다 — 화면 아래 초상화 대사(TutorialWidget)와 캐릭터 머리 위 말풍선(OverlaySpeechBubble).
    /// 그림은 SayneAssets/UI/Tutorial/Sprites 의 PNG 를 그대로 쓰고, 여기서는 임포트 설정만 맞춘다.
    /// 끝에 어드레서블 등록과 미리보기 저장까지 한 번에 돌린다.
    /// </summary>
    public static class TutorialWidgetBuilder
    {
        private const string Folder = "Assets/SayneAssets/UI/Tutorial";
        private const string SpriteFolder = Folder + "/Sprites";
        private const string CircleSolidPath = "Assets/SayneAssets/UI/Sprites/CircleSolid.png";
        ...
        {
            _bubble = ImportSprite("TutorialBubble", Vector4.one * BubbleBorder);
            _tail = ImportSprite("TutorialTail", Vector4.zero);
            _disc = ImportSprite("TutorialPortraitDisc", Vector4.zero);
            _ring = ImportSprite("TutorialPortraitRing", Vector4.zero);
            _nextMark = ImportSprite("TutorialNextMark", Vector4.zero);
            _circleSolid = AssetDatabase.LoadAssetAtPath<Sprite>(CircleSolidPath);
        ...
        private static void BuildTutorialWidget()
        {
            var root = FullScreenRoot("TutorialWidget");
            ...
            var widget = root.gameObject.AddComponent<TutorialWidget>();
        ...
        private static SpeechBubble Bubble(RectTransform parent, Vector2 anchor, Vector2 pivot, Vector2 pos, Vector2 size,
            ...
            HorizontalAlignmentOptions alignment, Vector2 nextMarkPos, float nextMarkScale, bool isWidthFitted)
        {
            ...
            var bubble = body.gameObject.AddComponent<SpeechBubble>();
        ...
        private static void CapturePreviews()
        {
            ...
            Capture($"{Folder}/TutorialWidget.prefab", Path.Combine(output, "tutorial-widget.png"), instance =>
            {
