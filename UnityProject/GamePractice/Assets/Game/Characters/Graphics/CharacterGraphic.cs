...
namespace Sayne
{
    ...
    {
        /// <summary>기본 공격은 타격 직전부터 재생하며, 전용 모션은 시작점을 재정의한다.</summary>
        protected virtual float AttackStartNormalized => 0.4f;
        ...
        protected virtual void Bind(Character character)
        {
            ...
            character.Equipment.CurrentWeapon
                .Subscribe(this, (weapon, self) => self.ApplyWeaponVisual(weapon != null ? weapon.Name : string.Empty))
                .AddTo(this);
        }

        /// <summary>무기 ID에 따라 비주얼을 바꾸는 자리. 빈 문자열 = 맨손. 지금은 전부 기본 비주얼이다.</summary>
        protected virtual void ApplyWeaponVisual(string weaponID)
        {
        }
        ...
        {
            if (state == CharacterState.Hit || state == CharacterState.Death)
            {
                _oneShotReturn?.Dispose();
                _oneShotReturn = null;
                _oneShotLayer = -1;
